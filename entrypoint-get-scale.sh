...
if [ "$running_processes" -eq 0 ] && [ "$pending_processes" -eq 0 ] && [ "$unhealthy_processes" -eq 0 ]; then
  echo "No processes found for service $INPUT_SERVICE in app $INPUT_APP"
  exit 1
fi
...
if [ -z "$desired" ] || [ -z "$running" ] || [ -z "$cpu" ] || [ -z "$memory" ]
then
  echo "Failed to parse scale output for service $INPUT_SERVICE in app $INPUT_APP"
  exit 1
fi
